...
time_out_hour = 13   # 매수 후 몇시간 경과시까지 매도가 안되면 자동 매도 할것인지
    ...
    try :
        ...
        if (now.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
            ...
            if bought_state == 0 :   # 매수가 없는 상태라면
                ...
                if ((Sell_Forced == 0) and                     (DF_test_ma['ema_o_l_consecutive_rise'][-(ma_duration_long + 1) : -1].sum() > 5) and                     (DF_test_ma['ema_o_s_consecutive_rise'][-(ma_short_under_duration + 1) : -1].sum() > (ma_short_under_duration - 2)) and                     ((DF_test_ma['ema_open_long'][-1] / DF_test_ma['ema_open_long'][-2]) > ratio_ema_long) and                     ((DF_test_ma['ratio_vol_to_aver'][-3] > buy_cri_vol_times) or (DF_test_ma['ratio_vol_to_aver'][-2] > buy_cri_vol_times)) and                     (DF_test_ma['ema_open_short'][-1] / DF_test_ma['ema_open_long'][-1] > ratio_ema_o_short_long_buy_1[0]) and (DF_test_ma['ema_open_short'][-1] / DF_test_ma['ema_open_long'][-1] < ratio_ema_o_short_long_buy_1[1]) and                     (DF_test_ma['ratio_ema_Not_buy_check'][-1] > ema_Not_buy_cri)) :
                    ...
                    buy_time = datetime.datetime.now() + datetime.timedelta(seconds = (time_factor * 3600))
        ...
        if bought_state == 1 :   # 매수가 되어 있는 상태라면
            ...
            # 매수 후 오랜시간 경과에 따른 강제 매도 영역 판단
            print(((datetime.datetime.now() + datetime.timedelta(seconds = (1 * 3650))) - (datetime.datetime.now() + datetime.timedelta(seconds = (0 * 3600)))).seconds / 3600)
            
            print ('\nTime_Out SELL condition checking')
            if (((datetime.datetime.now() + datetime.timedelta(seconds = (time_factor * 3600))) - buy_time).seconds / 3600) > time_out_hour :   # 매수 이후 time_out_hour 이상 시간이 경과했다면
                
                transaction_sell = upbit.sell_market_order(coin_inv, get_balance(coin_inv[4:]))   # 시장가에 매도
                time.sleep(5)
                print ('\nnow :', (datetime.datetime.now() + datetime.timedelta(seconds = (time_factor*3600))))
                print ('sell_transaction_result_BY_ Time_out _SELLING :', transaction_sell)
                bought_state = 0
                time.sleep(5)
